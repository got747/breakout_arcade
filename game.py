...
class Game(arcade.View):
    ...
    def setup(self, game_data={}):
        if not game_data:
            self.bat.center_x = config.STARTING_POINT_X_BAT
            self.bat.center_y = config.STARTING_POINT_Y_BAT

            self.layout_blocks_one()

            self.ball.center_x = config.STARTING_POINT_X_BALL
            self.ball.center_y = config.STARTING_POINT_Y_BALL
            self.ball.change_x = -config.SPEAD_BALL
            self.ball.change_y = -config.SPEAD_BALL
        else:
    ...
    def layout_blocks_one(self):
        centr_x = config.STARTING_POINT_FIRST_BLOCK_X
        centr_y = config.STARTING_POINT_FIRST_BLOCK_Y
        for lin in range(9):
            for col in range(7):
                block = Block(tuple_texturs=choice(config.TUPLE_PATH_IMAGE_BLOCKS))
                block.set_position(centr_x, centr_y)
                self.list_blocks.append(block)
                centr_x += config.BLOCK_STEP_X_AXIS
            centr_x = config.STARTING_POINT_FIRST_BLOCK_X
            centr_y += config.BLOCK_STEP_Y_AXIS
    ...
    def on_draw(self):
        ...
        if self.status:
            ...
            arcade.draw_text(
                "GAME OVER", config.WiDTH / 2.6, config.HEIGHT / 2, arcade.color.RED, 24
            )

    def on_key_press(self, symbol: int, modifiers: int):
        if symbol == arcade.key.LEFT:
            self.bat.change_x = -config.SPEAD_BAT
...
class StopView(arcade.View):
    def __init__(self):
        ...
        ui_text_label = arcade.gui.UITextArea(
            text="Click escape to finish or enter the name\
                    of the save and click return to exit.\
        ...
        self.v_box.add(
            self.ui_text_input.with_space_around(1).with_border(
...
class StartView(arcade.View):
    def __init__(self):
        ...
        ui_text_label = arcade.gui.UITextArea(
            text="Enter the name of the save or click return", width=450, height=60
        )
